...
## p.204 mpg 데이터와 midwest 데이터를 이용해 분석 문제를 해결해 보세요.
```{python}
# 1. mpg 데이터의 cty(도시연비)와 hwy(고속도로 연비)간에 어떤 관계가 있는지 알아보려고 합니다. x축은 hwy로 된 산점도를 만들어 보세요.
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
mpg = pd.read_csv("../../data/mpg.csv")
sns.scatterplot(data= mpg, x ='hwy', y = 'cty')
plt.show()

```
```{python}
# 2. 미국의 지역별 인구통계 정보를 담은 midwest.csv를 이용해 전체 인구와 아시아인 인구 간에 어떤 관계가 있는지 알아보려고 합니다. x축은 poptotal(전체인구), y축은 popasian(아시아인 인구)으로 된 산점도를 만들어 보세요. 전체 인구는 50만 명 이하, 아시아인 인구는 1만 명 이하인 지역만 산점도에 표시되게 설정하세요.
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
...
plt.show()
```
## p.211 mpg 데이터를 이용해 분석 문제를 해결해 보세요.
```{python}
